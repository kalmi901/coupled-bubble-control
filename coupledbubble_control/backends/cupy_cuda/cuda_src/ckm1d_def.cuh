...
void explicit_coupling(
    ...
    if (valid_lane and l_active){
        ...
        //if (lsid == 0 && t == 0){
        //    printf("luid %d, sum_rad_g0 %.12f \n", luid, sum_rad_g0);
        //    printf("luid %d, sum_rad_g1 %.12f \n", luid, sum_rad_g1);
        //    printf("luid %d, sum_trn_ng %.12f \n", luid, sum_trn_ng);
        //    printf("luid %d, sum_trn_g2 %.12f \n", luid, sum_trn_g2);
        //    printf("luid %d, sum_trn_g3 %.12f \n", luid, sum_trn_g3);
        //    printf("luid %d, dx[2]-pre %.12f \n", luid, dx[2]);
        //    printf("luid %d, dx[3]-pre %.12f \n", luid, dx[3]);
        //}
        ...
        //if (lsid == 0 && t == 0){
        //    printf("luid %d, cpf[0] %.12f \n", luid, cpf[0]);
        //    printf("luid %d, cpf[1] %.12f \n", luid, cpf[1]);
        //    printf("luid %d, cpf[2] %.12f \n", luid, cpf[2]);
        //    printf("luid %d, cpf[3] %.12f \n", luid, cpf[3]);
        //    printf("luid %d, dx[2]-post %.12f \n", luid, dx[2]);
        //    printf("luid %d, dx[3]-post %.12f \n", luid, dx[3]);
        //}
    }
...
void mat_vec_product(
    ...
    //}
    int tid = threadIdx.x;
    double xi = x[1];       // Local Bubble Position
    double av_rad = 0.0;
    double av_trn = 0.0;
    ...
    axv[0] = av_rad * cpf[0] + v[tid];
    axv[1] = av_trn + v[SPB * TILE + tid];
}
...
void implicit_coupling(
    ...
    if (valid_lane && l_active){
        ...
        s_vector_temp[             tid] = dx2;          // tid = lsid * TILE + luid
        s_vector_temp[SPB * TILE + tid] = dx3;
    ...
    for (int k = 0; k < LIN_MAX_ITER; k++){
        ...
        __syncthreads();
        //if (gsid == 0){
        //    printf("luid = %d, axv[0]:=r[0] = %.12f, axv[1]:=r[1] = %.12f \n", luid, axv[0], axv[1]);
        //    printf("lsid = %d, rnorm2 = %.12f \n", lsid, s_norm_temp[lsid]);
        //}
        ...
        if (valid_lane && l_active && !l_converged){
            s_vector_temp[             tid] += axv[0];          // tid = lsid * TILE + luid
            s_vector_temp[SPB * TILE + tid] += axv[1];
        }
        __syncthreads();
    ...
    // Update dx
    if (valid_lane && l_active){
        dx[2] = s_vector_temp[             tid];
        dx[3] = s_vector_temp[SPB * TILE + tid];
    }
}
...
void ode_fun(
    ...
    if (valid_lane & l_active){
        ...
        CPT(0, lsid, luid) = x0 * x22;
        CPT(1, lsid, luid) = x02 * x2;
        CPT(2, lsid, luid) = x02 * x2 * x3;
        CPT(3, lsid, luid) = x03 * x3;
        CPT(4, lsid, luid) = x03 * x32;
        CPT(5, lsid, luid) = x02;
        CPT(6, lsid, luid) = x03;
        CPT(7, lsid, luid) = x1;
