...
// System definitions
#ifndef RHS_HEADER
#define RHS_HEADER "ckm1d_def.cuh"
#endif
...
)
{
    ...
    if (tid < SPB){
        ...
        if (gid < NS){
            ...
            g_status_flags[gid]      = (int)terminated;
                ...
                terminated          = terminated || (abs(actual_event) < ETOL);
                s_actual_event_value[tid] = actual_event;
                s_terminated_system[tid]  = (int)terminated;
                g_status_flags[gid]       = (int)terminated * EVENT_TERMINAL;
            #endif
    ...
    for (int step = 0; step < kernel_steps; step++){
        ...
        if (valid_lane && l_active && (luid == 0)){
            ...
            #if NE > 0
            if (s_terminated_system[lsid] == 0 && s_actual_event_value[lsid] < ETOL){
