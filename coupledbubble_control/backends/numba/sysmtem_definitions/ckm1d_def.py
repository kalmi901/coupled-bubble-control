...
def make_model(model_spec, execution_spec, solver_spec,
    ...
    def explicit_coupling(
        ...
        if valid_lane and l_active:
            #xi = cpt[7, lsid, luid]         # Local Bubble Position
            xi = x[1]
            ...
            for j in range(UPS):
                ...
                #target_lane = (cuda.laneid // TILE) * TILE + j
                
                # -- Calculate the distance and the direction --
                xj = cpt[7, lsid, j]
                #xj = cuda.shfl_sync(current_mask, xi, target_lane)
                delta = (xi - xj)
    ...
    def mat_vec_product(
        ...
        axv[0] = av_rad * cpf[0] + v[0, lsid, luid]
        axv[1] = av_trn + v[1, lsid, luid]
    ...
    def implicit_coupling(
        ...
        if luid == 0:
            ...
            s_lin_converged[lsid] = 0 if (valid_lane and l_active) else 1      # type: ignore / terminated (inactive etc)
        ...
        if valid_lane and l_active:
            ...
            dx3 = dx[3]
            acc = dx2 * dx2 + dx3 * dx3
            s_vector_temp[0, lsid, luid] = dx2
            s_vector_temp[1, lsid, luid] = dx3
        ...
        for k in range(LIN_MAX_ITER):
            ...
            all_converged = (s_all_converged[0] >= SPB)
            if all_converged:
                #print("all_converged", k)
                break

        # Update dx
        if valid_lane and l_active:
            dx[2] = s_vector_temp[0, lsid, luid]
            dx[3] = s_vector_temp[1, lsid, luid]
