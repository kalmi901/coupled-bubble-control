...
def make_kernel(
    ...
    NCF = model_spec.ncf            # Number of coupling factors
    NCT = model_spec.nct            # Number of coupling terms
    ...
    def rkck45_coupled_solver(
        ...
        if tid < SPB:
            ...
            if gid < NS:
                ...
                if NE > 0:
                    ...
                    event_occured= (abs(actual_event) < ETOL)
        ...
        for step in range(kernel_steps):
            ...
            # ---------------- Event Handling -------------------
            # ---------------------------------------------------
            if NE > 0 and tid < SPB and (s_terminated_system[tid] == 0):    # type: ignore
            ...
            #if tid < SPB and (s_terminated_system[tid] == 0):       # type: ignore
            #    event_detected  = (s_event_detected[tid] == 1)      # type: ignore
            #    event_ratio     = s_event_ratio[tid]                # type: ignore
            #    actual_time     = s_actual_time[tid]                # type: ignore
            #    time_end        = s_time_end[tid]                   # type: ignore
            #    time_step       = s_time_step[tid]                  # type: ignore
            #    rem_time        = time_end - (actual_time + time_step)  # Ennyi idő lenne elfogadott lépés esetén hátra!
            #    sys_ok          = 1
            #    sys_ratio       = 1.0e30
            #    for u in range(UPS):
            #        ok = sh_ok[tid, u]          # type: ignore
            #        rr = sh_ratio[tid, u]       # type: ignore
            #        sys_ok = min(sys_ok, ok)
            #        sys_ratio = min(sys_ratio, rr)
                
            #    sh_ok[tid, 0] = sys_ok          # type: ignore --> system-wide information!
                
            #    fac_ok = min(5.0, max(0.2, 0.9 * sys_ratio**0.2))
            #    fac_bad = min(0.5, max(0.1, 0.9 * sys_ratio**0.25))
            #    multiplier = (fac_ok if sys_ok == 1 else fac_bad) if not event_detected else event_ratio
                #if tid == 0 and bid == 0:
                #    print("tid", tid, "sys_ratio", sys_ratio, "multiplier", multiplier, "actual_time", actual_time, "time_step", time_step)
            #    new_time_step = min(max(time_step * multiplier, MIN_STEP), MAX_STEP, rem_time)
                 
            #    s_time_step[tid] = new_time_step   # type: ignore
            #    s_actual_time[tid] = actual_time + (time_step if sys_ok == 1 and not event_detected else 0.0)   # type: ignore
            #    if NE > 0 and sys_ok:
            #        s_actual_event_value[tid] = s_next_event_value[tid]               # type: ignore
            #cuda.syncthreads()  # type: ignore
            ...
            if valid_lane and l_active and luid == 0:
                if sys_event_detected:
                    ...
                    #if (gsid == 0):
                    #    print("lsid", lsid, "sys_ratio", sys_ratio, "multiplier", (fac_ok if sys_ok else fac_bad), "l_actual_time", l_actual_time, "l_time_step", l_time_step)
                    if NE > 0 and sys_ok:
            ...
            if valid_system and l_active and luid == 0:
                ...
                if NE > 0 and (s_terminated_system[lsid] == 0): # type: ignore
                    if(abs(s_actual_event_value[lsid]) < ETOL):      # type: ignore
